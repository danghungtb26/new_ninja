...
// tạo hàm 
    ...
    public void lslog(String username, String ipv4) {
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            this.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = Util.toDateString(Date.from(Instant.now()));// nguyễn an
            String filename = "log_game/lslog/lslog_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename, true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("\ntài khoan : " + username + " đăng nhập " + " || Time : " + str + " || ip :" + ipv4
                    + " \n");
            bw.close();
            fw.close();
        } catch (Exception e) {
...
// tạo hàm
        ...
        try {
            synchronized (Server.LOCK_MYSQL) {
                ...
                if (red != null && red.first()) {
                    ...
                    final String password = red.getString("password");
    ...
    public void sendInfo() {
        Message m = null;
        try {
            this.c.hp = (long) this.c.getMaxHP();
            this.c.mp = (long) this.c.getMaxMP();
            m = new Message(-30);
    ...
    public void loadSkill() {
        ...
        try {
            ...
            m.writer().writeByte((int) this.c.get().speed());

            m.writer().writeInt((int) this.c.get().getMaxHP());
            m.writer().writeInt((int) this.c.get().getMaxMP());
    ...
    public void getMp() {
        ...
        try {
            if (this.conn != null) {
                ...
                m.writer().writeInt((int) this.c.get().mp);
                m.writer().flush();
                this.conn.sendMessage(m);
            }
        } catch (Exception e) {
    ...
    public void loadPpoint() {
        ...
        try {
            ...
            m.writer().writeShort((int) this.c.get().potential0);
            m.writer().writeShort((int) this.c.get().potential1);
            m.writer().writeInt((int) this.c.get().potential2);
            m.writer().writeInt((int) this.c.get().potential3);
            m.writer().flush();
            this.conn.sendMessage(m);
        } catch (Exception e) {
            e.printStackTrace();
                ...
                if (eff.template.type != 0 && eff.template.type != 12) {
                    if (eff.template.type != 4 && eff.template.type != 13 && eff.template.type != 17) {
                        if (eff.template.type == 23) {
                            m.writer().writeInt((int) this.c.get().hp);
                            m.writer().writeInt((int) this.c.get().getMaxHP());
                        }
                    } else {
                        m.writer().writeInt((int) this.c.get().hp);
        ...
        if (this.c.clone.ItemBody[10] != null) {
            switch (this.c.clone.ItemBody[10].id) {
                ...
                case 870:
                    ...
                    break;
                case 871:
                    this.mobMeCloneMessage(220, (byte) 1);
                    break;
                case 872:
                    this.mobMeCloneMessage(223, (byte) 1);
                    break;
            }
        } else {
