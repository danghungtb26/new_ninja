            ...
            if (!str.equals("")) {
                ...
                switch (menuId) {
                    case 1: {
                        ...
                        if (c != null && c.tileMap != null && c.tileMap.map != null && !c.tileMap.map.LangCo()
                                && c.tileMap.map.getXHD() == -1 && c.mapid != 111 && c.mapid != 133
                                && !c.tileMap.map.mapChienTruong() && !c.tileMap.map.mapLDGT()
                                && !c.tileMap.map.mapBossTuanLoc() && !c.tileMap.map.mapGTC()) {
                            if (p.c.level < 60 && c.tileMap.map.VDMQ()) {
                                p.conn.sendMessageLog("Trình độ của bạn chưa đủ để di chuyển tới đây");
                                return;
                            }
                            if (p.c.tileMap.map.mapGTC() || p.c.tileMap.map.mapChienTruong()
                                    || p.c.tileMap.map.id == 111 || p.c.tileMap.map.mapTuTien()) {
                                p.c.typepk = 0;
                    ...
                    case 119: {
                        ...
                        //     p.lockAcc();
                        // }
                        // if (p.c.quantityItemyTotal(386) >= 8 * soluong && p.c.quantityItemyTotal(393) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(394) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(395) >= 1 * soluong) {
                    ...
                    case 120: {
                        ...
                        //     p.lockAcc();
                        // }
                        // if (p.c.quantityItemyTotal(387) >= 8 * soluong && p.c.quantityItemyTotal(393) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(394) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(395) >= 1 * soluong) {
                    ...
                    case 121: {
                        ...
                        //     p.lockAcc();
                        // }
                        // if (p.c.quantityItemyTotal(388) >= 8 * soluong && p.c.quantityItemyTotal(393) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(394) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(395) >= 1 * soluong) {
                    ...
                    case 122: {
                        ...
                        //     p.lockAcc();
                        // }
                        // if (p.c.quantityItemyTotal(386) >= 8 * soluong && p.c.quantityItemyTotal(387) >= 8 * soluong
                        //         && p.c.quantityItemyTotal(388) >= 8 * soluong
                        //         && p.c.quantityItemyTotal(393) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(394) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(395) >= 1 * soluong) {
                    ...
                    case 2: {
                        ...
                        if (temp != null) {
                            ...
                            Service.chatNPC(p, (short) 0,
                                    "Người chơi này không ở trong cùng khu với con hoặc không tồn tại, ta không thể gửi lời mời!");
                        }
                        break;
                    }
                    case 10: {
                        ...
                        if (dun != null) {
                            if (dun.c1.id == p.c.id) {
                                if (dun.tienCuocTeam2 != 0 && dun.tienCuocTeam2 != tienCuoc) {
                                    Service.chatNPC(p, (short) 37, "Đối thủ của con đã đặt cược "
                                            + Util.getFormatNumber(dun.tienCuocTeam2) + " xu con hãy đặt lại đi!");
                                    return;
                                }
                                ...
                                dun.c2.p.sendAddchatYellow("Người chơi " + dun.c1.name + " đã được cược "
                                        + Util.getFormatNumber(dun.tienCuocTeam1) + " xu.");

                            } else if (dun.c2.id == p.c.id) {
                                if (dun.tienCuocTeam1 != 0 && dun.tienCuocTeam1 != tienCuoc) {
                                    Service.chatNPC(p, (short) 37, "Đối thủ của con đã đặt cược "
                                            + Util.getFormatNumber(dun.tienCuocTeam1) + " xu con hãy đặt lại đi!");
                                    return;
                                }
                                ...
                                p.c.upxuMessage(-tienCuoc);
                                Service.chatNPC(p, (short) 37,
                                        "Con đã đặt cược " + Util.getFormatNumber(dun.tienCuocTeam2) + " xu");
                                dun.c1.p.sendAddchatYellow("Người chơi " + dun.c2.name + " đã được cược "
                                        + Util.getFormatNumber(dun.tienCuocTeam2) + " xu.");
                            }

                            if (dun.tienCuocTeam1 != 0 && dun.tienCuocTeam2 != 0
                                    && dun.tienCuocTeam1 == dun.tienCuocTeam2 && dun.team1.size() > 0
                                    && dun.team2.size() > 0) {
                                if (dun.tienCuocTeam1 >= 1000000L) {
                                    Manager.serverChat("Server: ",
                                            "Người chơi " + dun.c1.name + " (" + dun.c1.level + ")"
                                                    + " đang thách đấu với " + dun.c2.name + " (" + dun.c2.level + "): "
                                                    + Util.getFormatNumber(dun.tienCuocTeam1)
                                                    + " xu tại lôi đài, hãy mau mau đến xem và cổ vũ.");
                                }
                                dun.startDun();
                            }
                        } else {
                    ...
                    case 110: {
                        ...
                        // if (p.c.quantityItemyTotal(638) >= 3 * soluong && p.c.quantityItemyTotal(639) >= 5 * soluong
                        //         && p.c.quantityItemyTotal(640) >= 1 * soluong
                        //         && p.c.quantityItemyTotal(641) >= 3 * soluong
                        //         && p.c.quantityItemyTotal(642) >= 2 * soluong) {
                    ...
                    case 111: {
                        ...
                        //     p.lockAcc();
                        // }
                        // if (p.c.quantityItemyTotal(638) >= 2 * soluong && p.c.quantityItemyTotal(639) >= 4 * soluong
                        //         && p.c.quantityItemyTotal(641) >= 2 * soluong
                        //         && p.c.quantityItemyTotal(642) >= 4 * soluong) {
                    ...
                    case 24: {
                        ...
                        //     return;
                        // }
                        // if (Long.parseLong(num) > p.luong) {
                        //     Service.chatNPC(p, (short) 57, "Bạn không có đủ lượng");
                        //     return;
                        // }
                        // p.upluongMessage(-Long.parseLong(num));
                        // p.c.upxuMessage(Long.parseLong(num) * 1L);
                        break;
                    }
                    case 25: {
                        // final String num = str.replaceAll(" ", "").trim();
                        ...
                        // }
                        // p.upluongMessage(-Long.parseLong(num));
                        // p.c.upyenMessage(Long.parseLong(num) * 1L);
                        break;
                    }
                    case 26: {
                        // final String num = str.replaceAll(" ", "").trim();
                        ...
                        //     return;
                        // }
                        // if (Long.parseLong(num) > p.c.xu) {
                        //     Service.chatNPC(p, (short) 57, "Bạn không có đủ xu");
                        //     return;
                        // }
                        // p.c.upxuMessage(-Long.parseLong(num));
                        // p.c.upyenMessage(Long.parseLong(num) * 1L);
                        break;
                    }
                        ...
                        try {
                            synchronized (Server.LOCK_MYSQL) {
                                ResultSet red = SQLManager.stat.executeQuery(
                                        "SELECT * FROM `gift_code` WHERE `code` LIKE '" + check + "' AND active = 1;");
                                if (red != null && red.first()) {
                                    ...
                                    if (isTime == 1) {
                                        if (Date.from(Instant.now())
                                                .compareTo(Util.getDate(red.getString("time"))) > 0) {
                                    ...
                                    if (red != null && red.first()) {
                                        ...
                                        if (p.c.level < gtLevel) {
                                                ...
                                                if (itemId[i] == -4) {
                                                    ...
                                                    if (data2 != null) {
                                                        ...
                                                        if (itemExpires[i] != -1) {
                                                            ...
                                                            itemup.expires = System.currentTimeMillis()
                                                                    + itemExpires[i];
                                                        } else {
                                            ...
                                            String sqlSET = "(" + p.id + ", '" + code + "', '"
                                                    + Util.toDateString(Date.from(Instant.now())) + "', '"
                                                    + Util.toDateString(Date.from(Instant.now())) + "', '"
                                                    + Util.toDateString(Date.from(Instant.now())) + "');";
                                            SQLManager.stat.executeUpdate(
                                                    "INSERT INTO `history_gift` (`player_id`,`code`,`time`, `created_at`, `updated_at`) VALUES "
                                                            + sqlSET);
                                        } else {
                                            p.conn.sendMessageLog(
                                                    "Lỗi xác nhận mã Gift code. Hãy liên hệ Admin để biết thêm chi tiết.");
                                        }
                                    }
                                    jar.clear();
                                    red.close();
                    ...
                    case 5: {
                        ...
                        if (temp != null) {
                            ...
                            if (_charTT != null && _charTT.id == p.c.id) {
                                ...
                                Service.startYesNoDlg(_charTT.p, (byte) 4, "Gia tộc " + p.c.clan.clanName
                                        + " muốn thách đấu với gia tộc của bạn. Bạn có đồng ý?");
                                ...
                                Service.chatNPC(p, (short) 32,
                                        "Tộc trưởng gia tộc đối phương không online hoặc không tồn tại. Không thể gửi lời mời.");
                            }
                        } else {
                        ...
                        try {
                            ...
                            if (joincoin <= p.coin) {
                                ...
                                if (4 <= (x + y + z) && (x + y + z) <= 10) {
                                    ...
                                    CheckTXCoin.checkTXCoinArrayList.add(new CheckTXCoin(p.c.name,
                                            "Kết quả Xỉu (" + (x + y + z) + ") thua " + joincoin + " coin",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    ...
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nSố coin hiện tại của bạn là : " + p.coin);
                                    CheckTXCoin.checkTXCoinArrayList.add(new CheckTXCoin(p.c.name,
                                            "Kết quả Tam Hoa (" + (x + y + z) + ") thua " + joincoin + " coin",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    SQLManager.stat.executeUpdate("UPDATE `player` SET `coin`=" + p.coin
                                            + " WHERE `id`=" + p.id + " LIMIT 1;");
                                    return;
                                    ...
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra là : " + x + " " + y + " " + z
                                                    ...
                                                    + "\nSố coin hiện tại của bạn là : " + p.coin);
                                    CheckTXCoin.checkTXCoinArrayList.add(new CheckTXCoin(p.c.name,
                                            "Kết quả Tài (" + (x + y + z) + ") ăn " + joincoin * 19 / 10 + " coin",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    SQLManager.stat.executeUpdate("UPDATE `player` SET `coin`=" + p.coin
                                            + " WHERE `id`=" + p.id + " LIMIT 1;");
                                    return;
                            ...
                            if (joincoinx <= p.coin) {
                                ...
                                if (4 > (x + y + z) || (x + y + z) > 10) {
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nSố coin hiện tại của bạn là : " + p.coin);
                                    CheckTXCoin.checkTXCoinArrayList.add(new CheckTXCoin(p.c.name,
                                            "Kết quả Tài (" + (x + y + z) + ") thua " + joincoinx + " coin",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    SQLManager.stat.executeUpdate("UPDATE `player` SET `coin`=" + p.coin
                                            + " WHERE `id`=" + p.id + " LIMIT 1;");
                                    return;
                                    ...
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nSố coin hiện tại của bạn là : " + p.coin);
                                    CheckTXCoin.checkTXCoinArrayList.add(new CheckTXCoin(p.c.name,
                                            "Kết quả Tam Hoa (" + (x + y + z) + ") thua " + joincoinx + " coin",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    SQLManager.stat.executeUpdate("UPDATE `player` SET `coin`=" + p.coin
                                            + " WHERE `id`=" + p.id + " LIMIT 1;");
                                    return;
                                    ...
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra là : " + x + " " + y + " " + z
                                                    ...
                                                    + "\nSố coin hiện tại của bạn là : " + p.coin);
                                    CheckTXCoin.checkTXCoinArrayList.add(new CheckTXCoin(p.c.name,
                                            "Kết quả Xỉu (" + (x + y + z) + ") ăn " + joincoinx * 19 / 10 + " coin",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    SQLManager.stat.executeUpdate("UPDATE `player` SET `coin`=" + p.coin
                                            + " WHERE `id`=" + p.id + " LIMIT 1;");
                                    return;
                            ...
                            if (joinxut <= p.c.xu) {
                                ...
                                if (4 <= (x + y + z) && (x + y + z) <= 10) {
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nKết quả : Xỉu"
                                            + "\nCòn cái nịt.");
                                    CheckTXXu.checkTXXuArrayList.add(new CheckTXXu(p.c.name,
                                            "Kết quả Xỉu (" + (x + y + z) + ") thua " + joinxut + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                } else if (x == y && x == z) {
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nCòn cái nịt.");
                                    CheckTXXu.checkTXXuArrayList.add(new CheckTXXu(p.c.name,
                                            "Kết quả Tam Hoa (" + (x + y + z) + ") thua " + joinxut + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                } else if ((x + y + z) > 10) {
                                    ...
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra là : " + x + " " + y + " " + z
                                                    ...
                                                    + "\nVề bờ");
                                    CheckTXXu.checkTXXuArrayList.add(new CheckTXXu(p.c.name,
                                            "Kết quả Tài (" + (x + y + z) + ") ăn " + joinxut * 19 / 10 + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                }
                            } else {
                            ...
                            if (joinxux <= p.c.xu) {
                                ...
                                if (4 > (x + y + z) || (x + y + z) > 10) {
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nCòn cái nịt.");
                                    CheckTXXu.checkTXXuArrayList.add(new CheckTXXu(p.c.name,
                                            "Kết quả Tài (" + (x + y + z) + ") thua " + joinxux + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                } else if (x == y && x == z) {
                                    Server.manager.sendTB(p, "Kết quả", "Số hệ thống quay ra : " + x + " " + y + " " + z
                                            ...
                                            + "\nCòn cái nịt.");
                                    CheckTXXu.checkTXXuArrayList.add(new CheckTXXu(p.c.name,
                                            "Kết quả Tam Hoa (" + (x + y + z) + ") thua " + joinxux + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                } else if ((x + y + z) <= 10 && (x + y + z) >= 4) {
                                    ...
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra là : " + x + " " + y + " " + z
                                                    ...
                                                    + "\nVề bờ");
                                    CheckTXXu.checkTXXuArrayList.add(new CheckTXXu(p.c.name,
                                            "Kết quả Xỉu (" + (x + y + z) + ") ăn " + joinxux * 19 / 10 + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                ...
                                if ((x + y + z + t) % 2 != 0) {
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra : " + x + " " + y + " " + z + " " + t
                                                    ...
                                                    + "\nKết quả : Lẻ"
                                                    + "\nCòn cái nịt.");
                                    CheckCLXu.checkCLXuArrayList.add(new CheckCLXu(p.c.name,
                                            "Kết quả Lẻ (" + (x + y + z + t) + ") thua " + joinxuc + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                } else if ((x + y + z + t) % 2 == 0) {
                                    ...
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra là : " + x + " " + y + " " + z + " " + t
                                                    ...
                                                    + "\nVề bờ");
                                    CheckCLXu.checkCLXuArrayList.add(new CheckCLXu(p.c.name,
                                            "Kết quả Chẵn (" + (x + y + z + t) + ") ăn " + joinxuc * 19 / 10 + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                }
                            } else {
                                ...
                                if ((x + y + z + t) % 2 == 0) {
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra : " + x + " " + y + " " + z + " " + t
                                                    ...
                                                    + "\nCòn cái nịt.");
                                    CheckCLXu.checkCLXuArrayList.add(new CheckCLXu(p.c.name,
                                            "Kết quả Chẵn (" + (x + y + z + t) + ") thua " + joinxul + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                } else if ((x + y + z + t) % 2 != 0) {
                                    ...
                                    Server.manager.sendTB(p, "Kết quả",
                                            "Số hệ thống quay ra là : " + x + " " + y + " " + z + " " + t
                                                    ...
                                                    + "\nVề bờ");
                                    CheckCLXu.checkCLXuArrayList.add(new CheckCLXu(p.c.name,
                                            "Kết quả Lẻ (" + (x + y + z + t) + ") ăn " + joinxul * 19 / 10 + " xu",
                                            Util.toDateString(Date.from(Instant.now()))));
                                    return;
                                }
                            } else {
                    ...
                    case 6: {
                        ...
                        long soluong = Integer.parseInt(str);
                        if (p.c.quantityItemyTotal(666) >= 2 * soluong && p.c.quantityItemyTotal(667) >= 2 * soluong
                                && p.c.quantityItemyTotal(668) >= 3 * soluong
                                && p.c.quantityItemyTotal(669) >= 1 * soluong) {
                    ...
                    case 7: {
                        ...
                        long soluong = Integer.parseInt(str);
                        if (p.c.quantityItemyTotal(666) >= 3 * soluong && p.c.quantityItemyTotal(667) >= 3 * soluong
                                && p.c.quantityItemyTotal(668) >= 4 * soluong
                                && p.c.quantityItemyTotal(670) >= 1 * soluong) {
                        ...
                        if (gtc != null) {
                            if (gtc.clan1.id == clanManager.id) {
                                if (gtc.tienCuoc2 != 0 && gtc.tienCuoc2 != tienCuoc) {
                                    Service.chatNPC(p, (short) 40, "Gia tộc đối thủ của con đã đặt cược "
                                            + Util.getFormatNumber(gtc.tienCuoc2) + " xu con hãy đặt lại đi!");
                                    return;
                                }
                                if (gtc.tienCuoc1 != 0) {
                                ...
                                if (gtc.gt2.size() > 0) {
                                    for (int i = 0; i < gtc.gt2.size(); i++) {
                                        gtc.gt2.get(i).p.sendAddchatYellow("Gia tộc " + clanManager.name
                                                + " đã được cược " + Util.getFormatNumber(gtc.tienCuoc1) + " xu.");
                                    }
                                }
                                ...
                                if (gtc.tienCuoc1 != 0 && gtc.tienCuoc1 != tienCuoc) {
                                    Service.chatNPC(p, (short) 40, "Gia tộc đối thủ của con đã đặt cược "
                                            + Util.getFormatNumber(gtc.tienCuoc1) + " xu con hãy đặt lại đi!");
                                    return;
                                }
                                if (gtc.tienCuoc2 != 0) {
                                ...
                                if (gtc.gt1.size() > 0) {
                                    for (int i = 0; i < gtc.gt1.size(); i++) {
                                        gtc.gt1.get(i).p.sendAddchatYellow("Gia tộc " + clanManager.name
                                                + " đã được cược " + Util.getFormatNumber(gtc.tienCuoc2) + " xu.");
                                    }
                                }
                            }

                            if (gtc.tienCuoc1 != 0 && gtc.tienCuoc2 != 0 && gtc.tienCuoc1 == gtc.tienCuoc2
                                    && gtc.gt1.size() > 0 && gtc.gt2.size() > 0) {
                                gtc.invite();
                            }
                        } else {
                            return;
                    ...
                    case 100: {
                        ...
                        if (num.length() > 10 || !Util.checkNumInt(num) || b < 0
                                || b >= Server.manager.rotationluck.length) {
                            return;
                        }
                    ...
                    case 101: {
                        ...
                        if (b == 0 && p.c.isTaskDanhVong == 1 && p.c.taskDanhVong[0] == 0
                                && p.c.taskDanhVong[1] < p.c.taskDanhVong[2]) {
                            p.c.taskDanhVong[1]++;
                        }
                        if (b == 1 && p.c.isTaskDanhVong == 1 && p.c.taskDanhVong[0] == 1
                                && p.c.taskDanhVong[1] < p.c.taskDanhVong[2]) {
                            p.c.taskDanhVong[1]++;
                        }
                    ...
                    case 9999: {
                        ...
                        if (temp != null) {
                            ...
                            if (banPlayer != null && !banPlayer.isAdmin()) {
                                Client.gI().kickSession(banPlayer.conn);
                                try {
                                    SQLManager.stat.executeUpdate(
                                            "UPDATE `player` SET `ban`=1 WHERE `id`=" + banPlayer.id + " LIMIT 1;");
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                p.conn.sendMessageLog(
                                        "Đã khoá tài khoản: " + banPlayer.username + " - nhân vật: " + temp.name);
                            } else {
